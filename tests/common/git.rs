...
pub fn create_test_commit(path: &Path, file_name: &str, content: &str, message: &str) -> Result<()> {
    ...
    if !commit_result.status.success() {
        anyhow::bail!("Failed to create commit: {}", String::from_utf8_lossy(&commit_result.stderr));
    }
...
pub fn create_multiple_repos(parent_dir: &Path, count: usize) -> Result<Vec<String>> {
    ...
    for i in 0..count {
        ...
        create_test_commit(&repo_path, "README.md", &format!("# Repo {}", i + 1), "Initial commit")?;
...
pub fn add_git_remote(path: &Path, remote_name: &str, url: &str) -> Result<()> {
    ...
    if !result.status.success() {
        anyhow::bail!("Failed to add remote: {}", String::from_utf8_lossy(&result.stderr));
    }
