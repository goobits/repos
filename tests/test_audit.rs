...
use goobits_repos::audit::hygiene::{HygieneStatistics, HygieneStatus, HygieneViolation, ViolationType};
...
fn test_truffle_statistics_initialization() {
    ...
    assert_eq!(stats.total_repos_scanned, 0, "Should start with 0 scanned repos");
    assert_eq!(stats.repos_with_secrets, 0, "Should start with 0 repos with secrets");
    assert_eq!(stats.total_secrets, 0, "Should start with 0 total secrets");
    assert_eq!(stats.verified_secrets, 0, "Should start with 0 verified secrets");
    assert_eq!(stats.unverified_secrets, 0, "Should start with 0 unverified secrets");
    assert!(stats.secrets_by_detector.is_empty(), "Should start with empty detector map");
    assert!(stats.failed_repos.is_empty(), "Should start with no failed repos");
    assert_eq!(stats.scan_duration, Duration::default(), "Should start with zero duration");
}
...
fn test_truffle_statistics_add_repo_result_no_secrets() {
    ...
    assert_eq!(stats.total_repos_scanned, 1, "Should increment scanned repos count");
    assert_eq!(stats.repos_with_secrets, 0, "Should not count repos with no secrets");
...
fn test_truffle_statistics_add_repo_result_with_unverified_secrets() {
    ...
    assert_eq!(stats.repos_with_secrets, 1, "Should count repo with secrets");
    ...
    assert_eq!(stats.unverified_secrets, 2, "Should have 2 unverified secrets");
    assert_eq!(*stats.secrets_by_detector.get("AWS").unwrap(), 1, "Should count AWS detector");
    assert_eq!(*stats.secrets_by_detector.get("GitHub").unwrap(), 1, "Should count GitHub detector");
}
...
fn test_truffle_statistics_add_repo_result_with_verified_secrets() {
    ...
    assert_eq!(stats.unverified_secrets, 1, "Should count 1 unverified secret");
    assert_eq!(*stats.secrets_by_detector.get("AWS").unwrap(), 2, "Should count both AWS secrets");
}
...
fn test_truffle_statistics_add_repo_failure() {
    ...
    assert_eq!(stats.total_repos_scanned, 1, "Should count failed repo as scanned");
    assert_eq!(stats.failed_repos.len(), 1, "Should track failed repo");
    assert_eq!(stats.failed_repos[0].0, "failed-repo", "Should store repo name");
    assert_eq!(stats.failed_repos[0].1, "TruffleHog not installed", "Should store error message");
}
...
fn test_truffle_statistics_accumulation_across_repos() {
    ...
    assert_eq!(stats.repos_with_secrets, 2, "Should count repos with secrets");
    assert_eq!(stats.total_secrets, 3, "Should count all secrets");
    assert_eq!(stats.verified_secrets, 1, "Should count verified secrets");
    assert_eq!(stats.unverified_secrets, 2, "Should count unverified secrets");
    assert_eq!(*stats.secrets_by_detector.get("AWS").unwrap(), 2, "Should aggregate AWS secrets");
    assert_eq!(*stats.secrets_by_detector.get("GitHub").unwrap(), 1, "Should count GitHub secret");
    assert_eq!(stats.failed_repos.len(), 1, "Should track failed repo");
}
...
fn test_truffle_statistics_generate_summary_no_secrets() {
    ...
    assert!(summary.contains("No secrets found"), "Should indicate no secrets");
}
...
fn test_truffle_statistics_generate_summary_with_verified_secrets() {
    ...
    assert!(summary.contains("3 VERIFIED secrets"), "Should highlight verified secrets");
    assert!(!summary.contains("unverified"), "Should prioritize verified in summary");
}
...
fn test_truffle_statistics_generate_summary_with_only_unverified() {
    ...
    assert!(summary.contains("5 unverified secrets"), "Should mention unverified secrets");
}
...
fn test_truffle_statistics_generate_detailed_report_text() {
    ...
    stats.failed_repos.push(("failed-repo".to_string(), "Timeout".to_string()));

    let report = stats.generate_detailed_report(false).expect("Report generation should succeed");

    assert!(report.contains("VERIFIED SECRETS FOUND (2)"), "Should show verified count");
    assert!(report.contains("UNVERIFIED SECRETS (3)"), "Should show unverified count");
    assert!(report.contains("SECRETS BY TYPE"), "Should have detector breakdown");
    ...
    assert!(report.contains("failed-repo - Timeout"), "Should show failure details");
}
...
fn test_truffle_statistics_generate_detailed_report_json() {
    ...
    stats.failed_repos.push(("repo1".to_string(), "Error".to_string()));

    let json_report = stats.generate_detailed_report(true).expect("JSON generation should succeed");

    // Parse JSON to verify structure
    let parsed: serde_json::Value = serde_json::from_str(&json_report)
        .expect("Should generate valid JSON");

    assert_eq!(parsed["summary"]["total_repos_scanned"], 5, "JSON should contain total_repos_scanned");
    assert_eq!(parsed["summary"]["repos_with_secrets"], 2, "JSON should contain repos_with_secrets");
    assert_eq!(parsed["summary"]["total_secrets"], 4, "JSON should contain total_secrets");
    assert_eq!(parsed["summary"]["verified_secrets"], 1, "JSON should contain verified_secrets");
    assert_eq!(parsed["summary"]["unverified_secrets"], 3, "JSON should contain unverified_secrets");
    assert_eq!(parsed["secrets_by_detector"]["AWS"], 2, "JSON should contain detector counts");
    assert!(parsed["failed_repos"].is_array(), "JSON should contain failed_repos array");
    assert_eq!(parsed["failed_repos"][0][0], "repo1", "JSON should contain failed repo name");
}
...
fn test_truffle_statistics_generate_detailed_report_empty() {
    ...
    assert!(report.trim().is_empty() || report.lines().count() < 3,
...
fn test_audit_statistics_initialization() {
    ...
    assert_eq!(stats.truffle_stats.total_repos_scanned, 0, "TruffleStats should be initialized");
...
fn test_secret_finding_structure() {
    ...
    assert_eq!(finding.file_path, "config/aws.yml", "Should store file path");
}
...
fn test_hygiene_statistics_initialization() {
    ...
    assert_eq!(stats.get_violation_repos().len(), 0, "Should start with no violations");
}
...
fn test_hygiene_statistics_update_clean() {
    ...
    assert!(summary.contains("0 with violations"), "Should show 0 violations");
}
...
fn test_hygiene_statistics_update_with_violations() {
    ...
    assert!(summary.contains("1 with violations"), "Should show 1 repo with violations");

    let violation_repos = stats.get_violation_repos();
    assert_eq!(violation_repos.len(), 1, "Should track 1 repo with violations");
...
fn test_hygiene_statistics_update_with_error() {
    ...
    assert!(summary.contains("0 clean"), "Should show 0 clean repos");
    assert!(summary.contains("0 with violations"), "Should show 0 violation repos");
...
fn test_hygiene_statistics_multiple_repos() {
    ...
    // Clean repo
    stats.update("repo1", "/path/repo1", &HygieneStatus::Clean, "clean", vec![]);
    ...
    stats.update("repo2", "/path/repo2", &HygieneStatus::Violations, "violations", violations);

    // Error repo
    stats.update("repo3", "/path/repo3", &HygieneStatus::Error, "error", vec![]);

    // Another clean repo
    stats.update("repo4", "/path/repo4", &HygieneStatus::Clean, "clean", vec![]);

    let summary = stats.generate_summary(4, Duration::from_secs(15));
    assert!(summary.contains("2 clean"), "Should count clean repos");
    assert!(summary.contains("1 with violations"), "Should count repos with violations");
...
fn test_hygiene_statistics_generate_detailed_summary_with_violations() {
    ...
    assert!(detailed.contains("HYGIENE VIOLATIONS"), "Should have violations header");
    assert!(detailed.contains("test-repo"), "Should show repo name");
    assert!(detailed.contains("3 violations"), "Should show total count");
    assert!(detailed.contains("1 gitignore"), "Should show gitignore count");
...
fn test_hygiene_statistics_generate_detailed_summary_with_failures() {
    ...
    assert!(detailed.contains("FAILED HYGIENE SCANS"), "Should have failures header");
    assert!(detailed.contains("failed-repo"), "Should show failed repo name");
...
fn test_hygiene_statistics_generate_detailed_summary_empty() {
    ...
    assert!(detailed.is_empty(), "Empty stats should produce empty detailed summary");
}
...
fn test_hygiene_violation_types() {
    ...
    assert_eq!(gitignore_violation.file_path, ".env", "Should store gitignore violation");
    assert_eq!(pattern_violation.file_path, "node_modules/pkg/file.js", "Should store pattern violation");
    assert_eq!(large_violation.size_bytes, Some(10_000_000), "Should store size for large files");
}
...
fn test_fix_options_fix_all() {
    ...
    assert!(options.target_repos.is_none(), "Should not have target repos by default");
}
...
fn test_fix_options_fix_all_with_target_repos() {
    ...
    assert_eq!(options.target_repos.unwrap(), targets, "Should store target repos");
}
...
fn test_fix_options_selective() {
    ...
    assert!(options.fix_gitignore, "Should allow selective gitignore fix");
...
async fn test_hygiene_statistics_with_git_repo() {
    ...
    let violation_repos = stats.get_violation_repos();
    assert_eq!(violation_repos.len(), 1, "Should track violation from git repo");
}
...
async fn test_truffle_statistics_with_multiple_detectors() {
    ...
    assert_eq!(stats.secrets_by_detector.len(), 5, "Should track all detector types");

    for detector in &detectors {
        assert_eq!(*stats.secrets_by_detector.get(*detector).unwrap(), 1,
            "Should count secret for {} detector", detector);
    }
}
...
fn test_truffle_statistics_report_sorting() {
    ...
    let report = stats.generate_detailed_report(false).expect("Report should succeed");

    // Check that detectors are sorted by count (descending)
    let lines: Vec<&str> = report.lines().collect();
    let detector_lines: Vec<&str> = lines.iter()
        .filter(|l| l.contains(" × "))
        .copied()
        .collect();

    // First should be Slack (15), then AWS (10), then GitHub (5)
    assert!(detector_lines[0].contains("15 × Slack"), "Should sort by count - Slack first");
    assert!(detector_lines[1].contains("10 × AWS"), "Should sort by count - AWS second");
    assert!(detector_lines[2].contains("5 × GitHub"), "Should sort by count - GitHub third");
}
...
fn test_hygiene_statistics_duration_formatting() {
    ...
    assert!(summary1.contains("1.5s"), "Should format sub-second duration");
...
fn test_comprehensive_audit_workflow() {
    ...
    hygiene_stats.update("clean-repo", "/path/clean", &HygieneStatus::Clean, "clean", vec![]);
    ...
    hygiene_stats.update("dirty-repo", "/path/dirty", &HygieneStatus::Violations, "violations", violations);

    // Repo 3: Scan failed
    truffle_stats.add_repo_failure("error-repo", "timeout");
    hygiene_stats.update("error-repo", "/path/error", &HygieneStatus::Error, "error", vec![]);

    // Verify combined statistics
    assert_eq!(truffle_stats.total_repos_scanned, 3, "Should scan all repos");
    assert_eq!(truffle_stats.verified_secrets, 1, "Should find verified secret");

    let hygiene_summary = hygiene_stats.generate_summary(3, Duration::from_secs(20));
    assert!(hygiene_summary.contains("1 clean"), "Should have clean repo");
    assert!(hygiene_summary.contains("1 with violations"), "Should have violations");
    assert!(hygiene_summary.contains("1 failed"), "Should have failure");

    // Generate reports
    let truffle_report = truffle_stats.generate_detailed_report(false)
        .expect("Truffle report should generate");
    assert!(truffle_report.contains("VERIFIED SECRETS"), "Should highlight verified secrets");
    assert!(truffle_report.contains("SCAN FAILURES"), "Should show failures");

    let hygiene_report = hygiene_stats.generate_detailed_summary();
    assert!(hygiene_report.contains("HYGIENE VIOLATIONS"), "Should show violations");
    assert!(hygiene_report.contains("FAILED HYGIENE SCANS"), "Should show failures");
}
