...
use crate::core::{
    init_command, set_terminal_title, set_terminal_title_and_flush, NO_REPOS_MESSAGE,
};
use anyhow::Result;
...
pub async fn handle_publish_command(
    ...
    if plan.skipped_count > 0 {
        ...
        let mut skip_msg = format!(
            "\r📦 Filtered to {visibility_type} repos only ({}) skipped)",
            plan.skipped_count
        );
